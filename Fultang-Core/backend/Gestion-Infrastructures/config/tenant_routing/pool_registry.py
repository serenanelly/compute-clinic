...
bibliothèque tierce nécessaire.
...
Concurrence : `threading.Lock()` PAR TENANT, PAR PROCESSUS PYTHON.
  - Portée : ce verrou ne protège QUE les requêtes traitées par le MÊME
    ...
    qu'UNE seule instance de Gestion-Infrastructures (vérifié dans
    docker-compose.yml — aucun mécanisme de scaling horizontal n'existe
    aujourd'hui) : cette portée est donc suffisante pour cette phase.
  - Limite explicite : si FullTang passe un jour à plusieurs instances
    de ce service (replicas, load balancer), CE verrou ne coordonnera
    PAS les instances entre elles. Chaque instance enregistrerait
    indépendamment son propre alias de connexion vers la MÊME base
    PostgreSQL — ce n'est pas dangereux en soi (PostgreSQL gère
    nativement des connexions concurrentes depuis plusieurs clients),
    seulement redondant. Un déploiement horizontal voudrait néanmoins un
    verrou distribué (verrou consultatif PostgreSQL `pg_advisory_lock`,
    ou Redis `SETNX`) pour éviter ce travail redondant à grande échelle
    — non implémenté ici, car aucune infrastructure de ce type n'existe
    encore dans FullTang et l'introduire maintenant serait une
    dépendance nouvelle non justifiée par le besoin actuel.
"""
import logging
import threading
...
class TenantDatabaseInactiveError(Exception):
    ...
    def __init__(self, tenant_id: str, current_status: str):
        ...
        super().__init__(
            f"La base du tenant {tenant_id} pour INFRASTRUCTURE n'est pas active (statut={current_status})."
        )
...
def _alias_for(tenant_id: str) -> str:
    ...
    return f"tenant_{tenant_id.replace('-', '')}_infrastructure"
...
def _build_connection_settings(info: TenantDatabaseInfo) -> dict:
    ...
    phase ultérieure : il suffirait de lire une valeur optionnelle sur
    la réponse du Registry ici, sans toucher au reste du mécanisme.
    """
    return {
...
def _admin_connection_params() -> dict:
    ...
    `default` (toutes les bases tenant de ce service vivent, dans cette
    phase, sur le même serveur PostgreSQL que `default` — aucun nouveau
    serveur introduit).
    """
...
def provision_database(tenant_id: str) -> dict:
    ...
    Appelle `migrate` SANS app_label : Django parcourt alors toutes les
    apps installées, mais c'est `TenantDatabaseRouter.allow_migrate`
    (router.py) qui décide RÉELLEMENT lesquelles s'appliquent sur cet
    alias — seule l'app tenant-scopée `infrastructures` (+ `migrations`,
    bookkeeping) y écrit quoi que ce soit, exactement comme pour la base
    `default`. Aucune duplication de la liste des apps tenant-scopées
    entre ce module et le router : une seule source de vérité
    (router.py) — même patron que Medical-Monitoring (plusieurs apps
    tenant-scopées).
