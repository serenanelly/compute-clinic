...
Communication service-to-service directe (Gestion-Infrastructures →
...
protocole introduit : Gestion-Infrastructures devient simplement un
appelant de confiance supplémentaire du même mécanisme `IsInternalService`.
...
# ce service EST "Gestion-Infrastructures", il n'y a pas d'ambiguïté
# possible (voir tenant-service/tenants/migrations/0003_seed_platform_services.py).
SERVICE_CODE = "INFRASTRUCTURE"
...
def resolve_tenant_database(tenant_id: str) -> TenantDatabaseInfo:
    ...
    service INFRASTRUCTURE pour `tenant_id`.
