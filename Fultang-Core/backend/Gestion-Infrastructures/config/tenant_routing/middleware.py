"""
middleware.py — Nettoyage du Tenant Context (Phase 6).

Le Tenant Context (context.py) est établi PENDANT le traitement de la
requête (par GatewayHeaderAuthentication ou AuthVerifyView), pas par un
middleware classique : au moment où le middleware Django "avant vue"
s'exécute, l'authentification DRF n'a pas encore eu lieu (elle se
produit à l'intérieur du dispatch de la vue). Ce middleware n'a donc
qu'UN rôle : garantir qu'un contexte établi pendant une requête ne
survit JAMAIS jusqu'à la requête suivante traitée par le même thread —
c'est la garantie d'isolation stricte entre requêtes demandée (tâche §4).

Le `try/finally` couvre aussi les cas d'exception non gérée dans la vue :
le nettoyage a lieu quoi qu'il arrive.
"""
from .context import get_current_tenant_context, reset_tenant_context, set_tenant_context
