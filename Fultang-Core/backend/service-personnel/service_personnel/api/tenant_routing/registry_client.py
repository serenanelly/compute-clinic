...
Communication service-to-service directe (service-personnel →
...
protocole introduit : `service-personnel` devient simplement un second
appelant de confiance du même mécanisme `IsInternalService`.
...
# ce service EST "service-personnel", il n'y a pas d'ambiguïté possible.
SERVICE_CODE = "PERSONNEL"
...
def resolve_tenant_database(tenant_id: str) -> TenantDatabaseInfo:
    ...
    service PERSONNEL pour `tenant_id`.
