...
TENANT CONTEXT vs PLATFORM/SYSTEM CONTEXT (voir tâche, §6) :
  - `TENANT_SCOPED_APPS` ('api', qui contient tout le personnel/les
    modèles métier de ce service) est routé selon le tenant courant —
  ...
  - Tout le reste (django.contrib.admin, auth, sessions, contenttypes —
    ...
    service-personnel), pas des données métier d'un tenant — un
...
class TenantDatabaseRouter:
    """DATABASE_ROUTERS : route les modèles de l'app `api` vers la base du tenant courant."""
    ...
    def allow_relation(self, obj1, obj2, **hints):
        """
        N'autorise une relation (ex: Personnel.service) que si les deux
