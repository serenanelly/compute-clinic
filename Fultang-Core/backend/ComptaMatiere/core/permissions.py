...
constant). ComptaMatiere devient ici un service supplémentaire exposant
un endpoint interne protégé par ce mécanisme déjà établi — aucun
nouveau protocole d'authentification introduit :

  - `POST /internal/provision-database/` : appelé par tenant-service
...
class HasFunctionalServiceEnabled(BasePermission):
    """
    Autorise l'opération uniquement si le `FunctionalService` COMPTA_MATIERE
    est activé pour le tenant COURANT — même mécanisme générique déjà
    déployé dans service-personnel/Medical-Monitoring/fultang-compta-financiere.
