"""
views.py — Endpoints internes de core (hors app métier).

Symétrique de `service-personnel/api/views.py::ProvisionDatabaseView` et
`Medical-Monitoring/backend/core/views.py::ProvisionDatabaseView` :
tenant-service appelle CET endpoint pour déclencher la création physique
réelle de la base PostgreSQL d'un tenant pour le service COMPTA_MATIERE,
puis l'initialisation de son schéma.
"""
...
class ProvisionDatabaseView(APIView):
    """
    POST /api/compta_matiere/internal/provision-database/
...
class FunctionalServiceInvalidateView(APIView):
    """
    POST /api/compta_matiere/internal/functional-services/invalidate/ —
    invalidation active du cache local `functional_service_cache`,
    symétrique de l'endpoint déjà déployé dans service-personnel/
    Medical-Monitoring/fultang-compta-financiere.
    """
    authentication_classes = []
    permission_classes = [IsInternalService]
