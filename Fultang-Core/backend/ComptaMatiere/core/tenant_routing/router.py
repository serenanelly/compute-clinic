...
aucune chambre, aucun service médical, aucune spécificité d'un
établissement — uniquement "quel app_label" et "quel tenant".

TENANT CONTEXT vs PLATFORM/SYSTEM CONTEXT (voir tâche, §6) :
  - `TENANT_SCOPED_APPS` ('comptabilite_matiere' — vérifié empiriquement
    contre `AppConfig.name = 'apps.comptabilite_matiere'`, qui n'a pas
    d'attribut `label` explicite : Django dérive alors le label du
    dernier composant du `name` pointé, soit 'comptabilite_matiere' —
    voir `python manage.py shell -c "... a.label ..."`, exécuté avant
    d'écrire ce fichier) est routé selon le tenant courant — jamais de
    valeur par défaut implicite.
  - Tout le reste (django.contrib.admin, auth, sessions, contenttypes —
    ...
    ComptaMatiere), pas des données métier d'un tenant — un "contexte
    plateforme" ne donne ACCÈS À AUCUNE base tenant : il ne fait que
    continuer d'utiliser 'default', exactement comme aujourd'hui. Il
    n'existe aucun mécanisme permettant à un code "plateforme" de
    choisir arbitrairement la base d'un tenant.
...
TENANT_SCOPED_APPS = {"comptabilite_matiere"}
...
class TenantDatabaseRouter:
    """DATABASE_ROUTERS : route les modèles de l'app comptabilite_matiere vers la base du tenant courant."""
    ...
    def allow_relation(self, obj1, obj2, **hints):
        """
        N'autorise une relation (ex: Rapport.materiel) que si les deux
