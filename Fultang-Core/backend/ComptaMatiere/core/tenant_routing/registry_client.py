...
Communication service-to-service directe (ComptaMatiere → tenant-service),
cohérente avec la décision déjà actée (Phase 4 : "les communications
service-to-service restent directes", Phase 2.1 : authentification par
jeton interne partagé `X-Internal-Service-Token`, déjà utilisé par la
Gateway pour `GET /tenants/resolve/`). Aucun nouveau protocole
introduit : ComptaMatiere devient simplement un appelant de confiance
supplémentaire du même mécanisme `IsInternalService`.
...
# ce service EST "Comptabilité Matière", il n'y a pas d'ambiguïté
# possible (voir tenant-service/tenants/migrations/0003_seed_platform_services.py).
SERVICE_CODE = "COMPTA_MATIERE"
...
def resolve_tenant_database(tenant_id: str) -> TenantDatabaseInfo:
    ...
    service COMPTA_MATIERE pour `tenant_id`.
