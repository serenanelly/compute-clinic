"""
tests_tenant_routing.py — Tests du chantier ComptaMatiere tenant-aware.

Même patron de tests que service-personnel/api/tests.py et
Medical-Monitoring/backend/core/tests_tenant_routing.py (Phase 6/7) —
adapté à l'unique app tenant-scopée de ce service
('comptabilite_matiere').

Exécution : `python manage.py test apps.comptabilite_matiere.tests.tests_tenant_routing`
(Postgres réel, comme le reste des tests ComptaMatiere — pas de sqlite ici).
"""
import threading
import uuid
...
class PoolRegistryTests(TestCase):
    ...
    def test_alias_suffix_is_compta_matiere_not_personnel_or_medical(self):
        """Régression : ComptaMatiere ne doit jamais produire le même alias qu'un autre service pour un même tenant_id."""
        self.assertTrue(self.alias.endswith("_compta_matiere"))
        self.assertNotIn("personnel", self.alias)
        self.assertFalse(self.alias.endswith("_medical"))
    ...
    def test_create_database_if_missing_is_idempotent(self):
        ...
        with patch('core.tenant_routing.pool_registry.psycopg2.connect', return_value=mock_conn):
            created = pool_registry_module._create_database_if_missing("tenant_x_compta_matiere")
    ...
    def test_provision_database_registers_alias_and_migrates_without_app_label(self):
        """
        Régression : le provisioning ne doit JAMAIS restreindre `migrate`
        à un seul app_label en dur (voir docstring de pool_registry.py) —
        c'est TenantDatabaseRouter.allow_migrate qui décide réellement.
        """
        mock_conn = MagicMock()
        mock_cursor = mock_conn.cursor.return_value.__enter__.return_value
        ...
        self.assertNotIn('comptabilite_matiere', mock_migrate.call_args.args)  # aucun app_label positionnel
...
class TenantDatabaseRouterTests(TestCase):
    ...
    def test_raises_when_no_context_established(self):
        ...
        def check():
            try:
                self.router.db_for_read(_FakeModel("comptabilite_matiere"))
                result["raised"] = False
            except TenantContextMissingError:
                result["raised"] = True
    ...
    def test_none_tenant_routes_to_default(self):
        set_tenant_context(None)
        self.assertEqual(self.router.db_for_read(_FakeModel("comptabilite_matiere")), "default")
    ...
    def test_real_tenant_routes_to_resolved_alias(self):
        ...
        with patch("core.tenant_routing.router.ensure_connection_alias", return_value="tenant_a_alias") as mock_ensure:
            self.assertEqual(self.router.db_for_read(_FakeModel("comptabilite_matiere")), "tenant_a_alias")
        mock_ensure.assert_called_once_with(ROUTE_TENANT_A)

    def test_non_tenant_scoped_app_is_never_routed(self):
        """`auth` n'est PAS dans TENANT_SCOPED_APPS : le router ne doit jamais tenter de résoudre un alias tenant pour lui."""
        set_tenant_context(ROUTE_TENANT_A)
        with patch("core.tenant_routing.router.ensure_connection_alias") as mock_ensure:
    ...
    def test_different_tenants_resolve_to_different_aliases(self):
        ...
        with patch("core.tenant_routing.router.ensure_connection_alias", side_effect=lambda t: f"alias_{t}"):
            alias_a = self.router.db_for_read(_FakeModel("comptabilite_matiere"))

        set_tenant_context(ROUTE_TENANT_B)
        with patch("core.tenant_routing.router.ensure_connection_alias", side_effect=lambda t: f"alias_{t}"):
            alias_b = self.router.db_for_read(_FakeModel("comptabilite_matiere"))
    ...
    def test_allow_migrate_tenant_scoped_app_everywhere(self):
        self.assertTrue(self.router.allow_migrate("default", "comptabilite_matiere"))
        self.assertTrue(self.router.allow_migrate("tenant_a_alias", "comptabilite_matiere"))
...
class GatewayHeaderAuthenticationTenantTests(TestCase):
    ...
    def test_authenticate_extracts_user_id_roles_and_tenant_id(self):
        request = self.factory.get(
            '/', HTTP_X_USER_ID='u1', HTTP_X_USER_ROLES='Gestionnaire,Directeur', HTTP_X_TENANT_ID=ROUTE_TENANT_A,
        )
        user, _ = self.auth.authenticate(request)
        self.assertEqual(user.id, 'u1')
        self.assertEqual(user.roles, ['Gestionnaire', 'Directeur'])
        self.assertEqual(user.tenant_id, ROUTE_TENANT_A)
        self.assertEqual(require_tenant_context().tenant_id, ROUTE_TENANT_A)

    def test_authenticate_without_tenant_header_leaves_tenant_id_none(self):
        request = self.factory.get('/', HTTP_X_USER_ID='u1', HTTP_X_USER_ROLES='Gestionnaire')
        user, _ = self.auth.authenticate(request)
...
class ProvisionDatabaseEndpointTests(APITestCase):
    def setUp(self):
        self.url = '/api/compta_matiere/internal/provision-database/'
    ...
    def test_valid_request_delegates_to_provision_database(self):
        with patch('core.views.provision_database', return_value={
            'database_name': 'tenant_x_compta_matiere', 'host': 'compta-matiere-db', 'port': 5432,
        }) as mock_provision:
            response = self.client.post(
                self.url, {'tenant_id': PROVISION_TENANT}, format='json', HTTP_X_INTERNAL_SERVICE_TOKEN='test-token',
            )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['database_name'], 'tenant_x_compta_matiere')
