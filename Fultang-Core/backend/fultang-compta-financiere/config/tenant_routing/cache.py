...
cache par service". Si FullTang passe à plusieurs instances de ce
service, chaque instance aura son propre cache indépendant (pas de
cohérence inter-instances) — acceptable ici car le cache ne contient
que des métadonnées de connexion peu volatiles, jamais des données
métier ; voir pool_registry.py pour la même limite appliquée au
