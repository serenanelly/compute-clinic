...
de l'utilisateur dans les headers HTTP :
  ...
  - X-Tenant-ID  : Tenant associé à l'utilisateur — absent si le compte
                   n'est rattaché à aucun tenant (pool non assigné).
...
(voir api-gateway/app/main.py::_strip_client_identity_headers — un
client ne peut jamais injecter X-Tenant-ID lui-même, la Gateway supprime
systématiquement toute tentative avant de reconstruire ces headers
depuis le seul JWT qu'elle a validé).

Chantier Medical-Monitoring tenant-aware : c'est ICI, au moment même où
...
jamais depuis une source moins fiable (même patron que service-personnel).
"""
