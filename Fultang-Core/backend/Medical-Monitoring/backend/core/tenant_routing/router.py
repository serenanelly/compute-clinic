"""
router.py — Dynamic Database Router pour Medical-Monitoring.
...
du modèle concerné. Ce router ne connaît RIEN du métier médical : aucun
patient, aucune visite, aucune spécificité d'un établissement —
uniquement "quel app_label" et "quel tenant".

TENANT CONTEXT vs PLATFORM/SYSTEM CONTEXT :
  - `TENANT_SCOPED_APPS` (patient, medical_workflow, patient_informations
    — les 3 apps métier de ce service) est routé selon le tenant
    courant — jamais de valeur par défaut implicite.
...
Le vrai Tenant Registry vit entièrement dans tenant-service, un service
séparé qui n'est pas concerné par ce router.

`tenant_id=None` (pool non assigné) route explicitement vers 'default'.
L'ABSENCE de tout Tenant Context (bug, accès hors requête — ex: un
signal Django exécuté sans capture explicite du tenant) lève
...
# Les 3 apps métier de ce service — SEULE source de vérité de cette
...
class TenantDatabaseRouter:
    ...
    def _resolve_alias(self) -> str:
        ...
        if tenant_context.tenant_id is None:
            # Pool non assigné — route explicite, documentée, vers la
            # base historique. Jamais un tenant "au hasard".
            return "default"
    ...
    def allow_relation(self, obj1, obj2, **hints):
        """
        N'autorise une relation (ex: Visite → Patient) que si les deux
