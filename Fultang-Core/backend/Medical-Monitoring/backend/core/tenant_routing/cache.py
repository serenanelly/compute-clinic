"""
cache.py — Cache TTL du mapping tenant → base de données.
...
invalidation active (webhook, signal) n'est implémentée à ce stade — si
la configuration d'un tenant change côté Registry, le changement met
jusqu'à TENANT_DB_CACHE_TTL_SECONDS avant d'être pris en compte ici
(même limite documentée que service-personnel).
...
- s'il existe une entrée en cache pour ce tenant, même expirée, elle est
  réutilisée (dégradation gracieuse), avec un avertissement loggué ;
- s'il n'existe AUCUNE entrée (premier accès de ce tenant), l'appel est
  ...
  deviner une base ici serait exactement le risque de fuite que ce
  mécanisme doit éliminer.

Portée : un dict par PROCESSUS Python (donc par conteneur/worker) — même
limite documentée que service-personnel/pool_registry.py.
"""
import logging
import threading
import time
