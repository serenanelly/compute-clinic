...
ensuite exactement comme s'ils avaient toujours existé. Vérifié
empiriquement pour Django 6.0.3 lors du chantier service-personnel ;
Medical-Monitoring tourne sur Django 5.1 — revérifié empiriquement ici
avant d'écrire ce module (voir tests : `test_ensure_connection_alias_*`
exécutés réellement contre ce projet, pas seulement copiés).
...
pas. Même mécanisme et mêmes limites que service-personnel.
...
Concurrence : `threading.Lock()` PAR TENANT, PAR PROCESSUS PYTHON.
  - Portée : ce verrou ne protège QUE les requêtes traitées par le MÊME
    processus/conteneur. Medical-Monitoring n'exécute qu'UNE seule
    instance (vérifié dans docker-compose.yml — aucun mécanisme de
    scaling horizontal) : cette portée est donc suffisante.
  - Limite explicite (identique à service-personnel) : un déploiement
...
class TenantDatabaseInactiveError(Exception):
    ...
    def __init__(self, tenant_id: str, current_status: str):
        ...
        super().__init__(
            f"La base du tenant {tenant_id} pour MEDICAL n'est pas active (statut={current_status})."
        )
...
def provision_database(tenant_id: str) -> dict:
    """
    Provisioning physique réel : crée la base PostgreSQL de `tenant_id`
    pour CE service si nécessaire, puis y initialise le schéma.

    Contrairement à service-personnel (une seule app tenant-scopée,
    'api'), Medical-Monitoring en a TROIS : `patient`, `medical_workflow`,
    `patient_informations`. Plutôt que d'enchaîner 3 appels `migrate`
    explicites (fragile si une 4e app tenant-scopée est ajoutée plus
    tard), on appelle `migrate` SANS app_label : Django parcourt alors
    toutes les apps installées, mais c'est `TenantDatabaseRouter.allow_migrate`
    (router.py) qui décide RÉELLEMENT lesquelles s'appliquent sur cet
    alias — seules les 3 apps tenant-scopées (+ `migrations`, bookkeeping)
    y écrivent quoi que ce soit, exactement comme pour la base `default`.
    Aucune duplication de la liste des apps tenant-scopées entre ce
    module et le router : une seule source de vérité (router.py).
