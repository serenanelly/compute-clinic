...
export function QuittancesValideesPage() {
  ...
  const loadQuittances = async () => {
    ...
    try {
      ...
      const data = await getQuittancesValidees(filters);
      setQuittances(data || []);
    } catch (err) {
  ...
  const handlePrintQuittance = (quittance) => {
    const printContent = `
      ...
      <body>
          ...
          <div class="title-box">
              QUITTANCE DE CAISSE N° ${quittance.numero || quittance.numero_quittance}
          </div>
          <div class="meta-info">
              <div class="meta-block">
                  ...
                  <div class="meta-row">
                      ...
                      <span class="value">${quittance.patient_nom || `Patient #${quittance.patient_id || 'N/A'}`}</span>
                  </div>
              </div>
              <div class="meta-block">
                  ...
                  <div class="meta-row">
                      ...
                      <span class="value">${new Date(quittance.date_creation || quittance.date_paiement).toLocaleString('fr-FR')}</span>
                  </div>
                  <div class="meta-row">
                      <span class="label">Mode:</span>
                      <span class="value" style="text-transform: capitalize;">${(quittance.mode_paiement || 'especes').replace('_', ' ')}</span>
                  </div>
                  <div class="meta-row">
                      <span class="label">Motif:</span>
                      <span class="value">${quittance.motif || quittance.Motif || '—'}</span>
                  </div>
              </div>
          </div>
          <div class="amount-section">
              <div class="amount-title">Montant perçu</div>
              <div class="amount-value">${new Intl.NumberFormat('fr-FR').format(quittance.montant || quittance.Montant_paye || 0)} FCFA</div>
          </div>
  ...
  const filteredQuittances = quittances.filter(q => {
    const num = q.numero || q.numero_quittance || '';
    const motif = q.motif || q.Motif || '';
    const patient = q.patient_nom || '';

    const matchesSearch =
      num.toLowerCase().includes(searchTerm.toLowerCase()) ||
      motif.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    if (dateRange) {
      const qDate = new Date(q.date_creation || q.date_paiement);
  ...
  const columnsJournal = [
    {
      title: 'N° Quittance',
      key: 'numero',
      render: (_, r) => <span className="font-semibold text-primary-end">{r.numero || r.numero_quittance}</span>
    },
    {
      title: 'Patient',
      key: 'patient',
      render: (_, r) => <span className="font-medium text-gray-700">{r.patient_nom || `Patient #${r.patient_id || '—'}`}</span>
    },
    {
      title: 'Date Paiement',
      key: 'date',
      render: (_, r) => new Date(r.date_creation || r.date_paiement).toLocaleString('fr-FR')
    },
    {
      title: 'Motif',
      key: 'motif',
      ellipsis: true,
      render: (_, r) => r.motif || r.Motif || '—'
    },
    {
      title: 'Mode',
    ...
    },
    {
      ...
      key: 'montant',
      align: 'right',
      render: (_, r) => <span className="font-extrabold text-emerald-600">{formatFCFA(r.montant || r.Montant_paye)}</span>
    },
    {
      title: 'Actions',
      key: 'actions',
  ...
  return (
    <CashierLayout>
        <CashierPageHeader
          ...
          subtitle="Consultation, ventilation comptable et export des encaissements de caisse."
          actions={(
            <Space>
              <Button
                type="primary"
              ...
              children: (
                <div className="space-y-4 pt-2">
                  ...
                  <Table
                    ...
                    rowKey={(r) => r.id || r.idQuittance || r.numero}
                    loading={loading}
