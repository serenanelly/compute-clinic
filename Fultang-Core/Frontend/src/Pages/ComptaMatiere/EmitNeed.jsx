...
function getMaterielName(materiel) {
    return materiel?.nom_Materiel || materiel?.nom_materiel || materiel?.name || "";
}
...
export function EmitNeed() {
    ...
    const personnelName = useMemo(() => {
        const fromUser = `${userData?.prenom || ""} ${userData?.nom || ""}`.trim();
        if (fromUser) return fromUser;
        return localStorage.getItem("user_name") || "Comptable Matière";
    }, [userData]);
    ...
    function updateNeedItem(id, field, value) {
        setNeedItems(prev => prev.map(item =>
            item.id === id ? { ...item, [field]: value } : item
        ));
    }

    // Garder personnel_id aligné avec l'utilisateur connecté
    useEffect(() => {
        if (personnelId) {
            localStorage.setItem("personnel_id", String(personnelId));
        }
        if (personnelName) {
            localStorage.setItem("user_name", personnelName);
        }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    readOnly
                                />
                                <p className="text-xs text-gray-500 mt-1">Utilisateur : {personnelName}</p>
                            </div>
...
function NeedItemRow({ item, index, materiels, onUpdate, onRemove, canRemove }) {
    ...
    function handleMaterialChange(value) {
        ...
        if (value.length >= 2) {
            const filtered = materiels.filter(m =>
                getMaterielName(m).toLowerCase().includes(value.toLowerCase())
            ).slice(0, 8);
    ...
    function selectSuggestion(materiel) {
        const name = getMaterielName(materiel);
        if (!name) return;
        onUpdate(item.id, 'material', name);
        setSuggestions([]);
        setShowSuggestions(false);
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div className="md:col-span-2 relative">
                    ...
                    <input
                        ...
                        onFocus={() => {
                            if (item.material.length >= 2 && suggestions.length > 0) {
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                        <div className="absolute z-20 w-full bg-white border border-gray-300 rounded-lg mt-1 shadow-lg max-h-48 overflow-y-auto">
                            {suggestions.map(m => {
                                const name = getMaterielName(m);
                                return (
                                    <div
                                        key={m.idMateriel || m.materiel_ptr_id || name}
                                    ...
                                    >
                                        ...
                                        <p className="text-xs text-gray-500">Stock: {m.quantite_stock ?? 0}</p>
                                    </div>
                                );
                            })}
                        </div>
